...
export function RequestsTable({
  ...
  const [manualForm, setManualForm] = useState({
    ...
    itemsText: "1x Perceuse Visseuse Sans Fil Pro 18V, 5x Boîte 500 Vis Inox",
  });
  ...
  const handleClearAllRequests = async () => {
    try {
      ...
      if (res.ok) {
        ...
        showToast("Toutes les demandes ont été vidées.");
        onRequestsUpdated();
      }
    } catch (err) {
  ...
  const handleManualRequestSubmit = async (e: React.FormEvent) => {
    ...
    const parsedData: ParsedRequest = {
      ...
      summary: `Demande manuelle (Téléphone/Accueil) : ${manualForm.raw_content.slice(0, 80)}...`,
    };
    ...
    try {
      const res = await fetch("/api/requests", {
        ...
        body: JSON.stringify({
          client_name: manualForm.client_name.trim(),
          client_email: manualForm.client_email.trim() || null,
          raw_content: `[Saisie Manuelle]\nClient: ${manualForm.client_name}\nTél: ${manualForm.client_phone || "Non renseigné"}\n\n${manualForm.raw_content}`,
      ...
      if (res.ok) {
        showToast(`Demande de ${manualForm.client_name} créée avec succès.`);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 font-normal mt-0.5">
              Flux d'emails analysés en temps réel par l'IA et prêts pour validation
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          {requests.length > 0 && (
            <button
              ...
              title="Vider les demandes de test"
            >
        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="bg-black/30 border-b border-white/[0.06] text-slate-400 font-medium uppercase tracking-wider text-[10px]">
              ...
              <th className="py-3 px-5">Intention</th>
              <th className="py-3 px-5">Urgence</th>
              <th className="py-3 px-5">Statut</th>
              <th className="py-3 px-5">Articles Détectés</th>
              <th className="py-3 px-5">Date de Réception</th>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-10 text-center text-slate-500">
                  {requests.length === 0
                    ? "Aucune demande en attente. Utilisez 'Nouvelle Demande Manuelle' ou le testeur dans les Paramètres."
                    : "Aucune demande ne correspond à vos filtres."}
                </td>
              </tr>
            ) : (
              filtered.map((req) => {
                const isManual = req.status === "needs_manual_handling";
                return (
                  <tr
                    key={req.id}
                  ...
                  >
                    ...
                    <td className="py-3.5 px-5">
                      <div className="font-semibold text-slate-100 group-hover:text-blue-400 transition-colors">
                        {req.client_name || (
                          <span className="text-slate-500 italic font-normal">Non identifié</span>
                        )}
                      </div>
                      <div className="text-xs text-slate-400 mt-0.5 font-mono">
                        {req.client_email || <span className="text-slate-600">Pas d'email</span>}
                      </div>
                    </td>

                    {/* Intention (Clean text) */}
                    <td className="py-3.5 px-5">
                      <IntentBadge intent={req.parsed_data?.intent} />
                    </td>

                    {/* Urgence (Subtle dot indicator) */}
                    <td className="py-3.5 px-5">
                      <UrgencyBadge urgency={req.parsed_data?.urgency} />
                    </td>

                    {/* Statut (The ONLY pill badge) */}
                    <td className="py-3.5 px-5">
                      <StatusBadge status={req.status} />
                    </td>

                    {/* Articles Détectés (Clean plain readable text) */}
                    <td className="py-3.5 px-5 max-w-xs">
                      {req.parsed_data?.requested_items && req.parsed_data.requested_items.length > 0 ? (
                        ...
                        <span className="text-slate-500 text-xs">
                          {isManual ? "Nécessite lecture" : "Aucun article"}
                        </span>
                      )}
                    </td>

                    {/* Date (Single line strictly) */}
            ...
            <div className="bg-black/30 border-b border-white/[0.07] px-5 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-white">
                  Création d'une Demande Manuelle (Téléphone / Accueil)
                </h3>
              </div>
              <button
            ...
            <form onSubmit={handleManualRequestSubmit} className="p-5 space-y-4 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: 2x Perceuse 18V, 10x Lot Disques Diamant"
                  className="w-full h-9 bg-black/40 border border-white/[0.08] rounded-lg px-3 text-slate-200 focus:outline-none focus:border-blue-500"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Notes de l'appel téléphonique ou détails de la commande..."
              ...
              <div className="pt-2 flex justify-end gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-[#2563EB] hover:bg-blue-600 text-white font-semibold shadow-sm disabled:opacity-50"
                >
        ...
        <div className="fixed inset-0 z-50 overflow-hidden flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-[#111318] border border-white/[0.08] rounded-xl p-5 shadow-2xl space-y-4">
            <div className="flex items-center gap-3 text-rose-400">
              ...
              <h3 className="text-sm font-semibold text-white">Confirmer le vidage</h3>
            </div>
            <p className="text-xs text-slate-300 leading-relaxed">
              Voulez-vous supprimer toutes les demandes de la liste ? Cette action réinitialisera votre tableau de bord.
            </p>
